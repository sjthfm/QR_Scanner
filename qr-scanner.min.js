/*
 @asset(https://rawcdn.githack.com/sjthfm/QR_Scanner/master/qr-scanner-worker.min.js) */
'use strict';
export default class QrScanner {
    ...
    _scanFrame() {
        ...
        requestAnimationFrame(() => {
            QrScanner.scanImage(this.$video,
                this._sourceRect, this._qrWorker, this.$canvas, true).then(this._onDecode, (error) => {
    ...
    _getMatchingCameraStream(constraintsToTry) {
        if (constraintsToTry.length ===
            0) return Promise.reject("Camera not found.");
    ...
    start() {
        ...
        return this._getCameraStream("environment", true).catch(() => {
            facingMode = "user";
    ...
    static scanImage(imageOrFileOrUrl, sourceRect = null, worker =
        ...
        if (sourceRect && alsoTryWithoutSourceRect) return promise.catch(() =>
            QrScanner.scanImage(imageOrFileOrUrl, null, worker, canvas, fixedCanvasSize));
        else return promise
    }
    static _getImageData(image, sourceRect = null, canvas = null, fixedCanvasSize = false) {
        ...
        const sourceRectHeight = sourceRect && sourceRect.height ? sourceRect.height :
    ...
    static _loadImage(imageOrFileOrUrl) {
        if (imageOrFileOrUrl instanceof HTMLCanvasElement || imageOrFileOrUrl instanceof HTMLVideoElement || window.ImageBitmap && imageOrFileOrUrl instanceof window.ImageBitmap || window.OffscreenCanvas && imageOrFileOrUrl instanceof window.OffscreenCanvas) return Promise.resolve(imageOrFileOrUrl);
        else if (imageOrFileOrUrl instanceof Image) return QrScanner._awaitImageLoad(imageOrFileOrUrl).then(() => imageOrFileOrUrl);
...
QrScanner.WORKER_PATH = "https://rawcdn.githack.com/sjthfm/QR_Scanner/master/qr-scanner-worker.min.js";

//# sourceMappingURL=https://raw.githack.com/nimiq/qr-scanner/master/qr-scanner.min.js.map
